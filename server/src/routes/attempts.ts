...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      const answerMap = new Map<string, number | undefined>(
        answers.map((a: any) => [a.questionId.toString(), a.selectedAnswer as number])
      );

      const processedAnswers: Array<{ questionId: string; selectedAnswer: number }> = [];
    ...
    res.json({
      attempts: await Promise.all(
        attempts.map(async (attempt) => {
          ...
          const quiz = useInMemoryDB
            ? await findQuizById(attempt.quizId as string)
            : await findQuizById(quizId);
...
router.get('/quiz/:quizCode/leaderboard', authenticateToken, async (req: AuthRequest, res) => {
  try {
const quiz = useInMemoryDB
